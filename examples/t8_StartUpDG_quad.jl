...
# coordinates
VX = [zeros(4) for _ in 1:num_elements]
VY = [zeros(4) for _ in 1:num_elements]
...
p = [1, 3, 2, 4]
VX_permuted = map(vx -> vx[p], VX)
VY_permuted = map(vx -> vx[p], VY)
md = MeshData((VX_permuted, VY_permuted), FToF, nonconforming_faces, rd)
